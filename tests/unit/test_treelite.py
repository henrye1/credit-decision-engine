...
def test_parse_and_compile(basic_treelite_config: str):
    ...
    assert set(tree.nodes.keys()) == {"0", "1", "2", "3a96d1da-b922-4c4d-bf24-0f7119b7eb72", "ab018e87-cb89-4bc4-8612-5cab02280f09"}
    ...
    assert (compiled_tree.output_dataframe_mapping.dtypes == ["O", "O", "O"]).all()
    leaf_priorities = [
        compiled_tree.output_priority_mapping[compiled_tree.node_id_mapping["1"]],
        compiled_tree.output_priority_mapping[compiled_tree.node_id_mapping["3a96d1da-b922-4c4d-bf24-0f7119b7eb72"]],
        compiled_tree.output_priority_mapping[compiled_tree.node_id_mapping["ab018e87-cb89-4bc4-8612-5cab02280f09"]],

    ]
    assert (
        leaf_priorities[0] > leaf_priorities[1] > leaf_priorities[2]
    )
    import numpy as np
    ...
    assert (priorities == np.array([[leaf_priorities[0]], [leaf_priorities[2]], [leaf_priorities[0]], [leaf_priorities[1]]])).all()
    ...
    assert (df_outs.description.values == np.array([
        "Output of node 1","Output of node ab018e87-cb89-4bc4-8612-5cab02280f09","Output of node 1","Output of node 3a96d1da-b922-4c4d-bf24-0f7119b7eb72"
    ])).all()
...
def test_multiple_disconnected_trees(multiple_trees_config: str):
    ...
    assert set(tree.nodes.keys()) == {
        "tree1_root", "tree1_leaf1", "tree1_leaf2",
        "tree2_root", "tree2_leaf1", "tree2_leaf2"
    }
...
def test_categorical_features(multiple_trees_config: str):
    ...
    predictions = treelite.gtil.predict(compiled_tree.model, X)
    outputs = compiled_tree.leaf_output_mapping[predictions[:,1,0].astype(int)]  # Check second tree only
...
def test_default_left_behavior(multiple_trees_config: str):
    ...
    expected_outputs = np.array([
        [1, 2],  # NaN in first feature -> right child of tree1, normal left path in tree2
        [0, 2],  # Normal left path in tree1, NaN in second feature -> left child of tree2
    ])
    assert (outputs == expected_outputs).all()
...
def test_create_and_run_hamilton_graph(basic_treelite_config: str):
    ...
    assert set(tree_driver.graph.nodes.keys()) == {
        'tree', 'tree.all_tree_paths', 'tree.formatted_inputs', 
        'tree.highest_priority_index', 'tree.index', 'tree.prioritized_outputs', 
        'tree.prioritized_tree_paths', 'tree.tree_results', 'tree.tree_path_keys', 'feature_0', 'feature_1'
    }
    data = [
            [1, 0],
            [2, 0],
    ...
    formatted_inputs = tree_driver.raw_execute(inputs=pd.DataFrame(
        data=data,
        columns=tree.features
    ), final_vars=["tree.formatted_inputs"])["tree.formatted_inputs"]
...
def test_highest_priority_index(basic_treelite_config: str):
    ...
    results = tree_driver.raw_execute(
        inputs=input_data,
        final_vars=["tree.highest_priority_index"]
...
def test_prioritized_outputs(basic_treelite_config: str):
    ...
    results = tree_driver.raw_execute(
        inputs=input_data,
        final_vars=["tree.prioritized_outputs"]
...
def test_get_output(basic_treelite_config: str):
    ...
    assert results.loc['b', 'description'] == "Output of node ab018e87-cb89-4bc4-8612-5cab02280f09"
    assert results.loc['c', 'description'] == "Output of node 1"
    assert results.loc['d', 'description'] == "Output of node 3a96d1da-b922-4c4d-bf24-0f7119b7eb72"
...
def test_all_tree_paths(basic_treelite_config: str):
    ...
    results = tree_driver.raw_execute(
        inputs=input_data,
        final_vars=["tree.all_tree_paths", "tree.tree_path_keys"]
    )
    ...
    assert paths.shape == (4, 1, 2)  # 4 samples, 1 tree, 2 condition nodes (root and categorical test)
    ...
    assert set(path_labels) == set(condition_labels)  # Root node and categorical test node
...
def test_prioritized_tree_paths(basic_treelite_config: str):
    ...
    results = tree_driver.raw_execute(
        inputs=input_data,
        final_vars=["tree.prioritized_tree_paths", "tree.all_tree_paths", "tree.tree_path_keys", "tree.highest_priority_index"]
    )
...
def test_tree_path_keys(basic_treelite_config: str):
    ...
    assert tree.nodes[path_labels[0]].node_type in ["numerical_test_node", "categorical_test_node"]
    assert tree.nodes[path_labels[1]].node_type in ["numerical_test_node", "categorical_test_node"]
...
def test_index_handling(basic_treelite_config: str):
    ...
    input_data = {
        "feature_0": np.array([1.0, 2.0]),
        "feature_1": np.array([0.0, 1.0])
    }
