...
from .node_types import NodeData, PositionedNode, LeafNode
from .variable_types import VariableMap, PlaceHolderVariable
from .edge import MultiSourceEdge
from .compiler import compile
from .output_schema import OutputSchema
from logging import getLogger

if t.TYPE_CHECKING:
    from ..tree import ValidationResults
...
class Tree(VariableNode):
    ...
    # @model_validator(mode="after")
    # def validate_tree_can_compile(self) -> "Tree":
    #     """Validate that compile() can be run without errors"""
    #     _ = self.compile()
    #     return self
    ...
    def node_lookup(self):
        if self._node_lookup is None:
            self._node_lookup = {n.id: n.data for n in self.nodes}
        return self._node_lookup

    def _output_df_and_map_v0(self):
        try:
            ...
            for n in self.nodes:
                if n.data.NODE_TYPE != "leaf":
                    continue
                node_mapping[n.id] = n.data.leaf_value
            output_df_map = pd.DataFrame(
                data=self.tree_output.data,
                columns=self.tree_output.columns,
                dtype=self.tree_output.dtype,
                index=range(-1, len(self.tree_output.data) - 1, 1),
            )
        except ValueError as e:
            raise ValueError("Could not create output dataframe") from e
        return output_df_map, node_mapping

    def _output_df_and_map_v1(self):
        node_mapping = {}
        data = []
        for n in self.nodes:
            if n.data.NODE_TYPE != "leaf":
            ...
            if n.data.output_data is None:
                assert (
                    self.output_schema.has_default_values()
                ), "Default value cannot be none when leaf nodes have no value set."
                node_mapping[n.id] = -1
                continue
            node_mapping[n.id] = len(data)
            data.append(n.data.output_data)
        data = [self.output_schema.default_values or {}] + data
        output_df = pd.DataFrame(
            data, columns=self.output_schema.columns(), index=range(-1, len(data) - 1)
        ).astype(self.output_schema.dtypes())
        return output_df, node_mapping

    def output_df_and_map(self):
        if self.node_output_format_version == 0:
            return self._output_df_and_map_v0()
        elif self.node_output_format_version == 1:
            return self._output_df_and_map_v1()
        raise ValueError(
            f"Expected node output format version 0, 1 but found {self.node_output_format_version}."
        )

    def output_df(self):
        return self.output_df_and_map()[0]

    def compile(self):
        # Create pseudo leaf nodes for missing edges before compilation
        # self._create_pseudo_leaf_nodes()
        return compile(self)
    ...
    def verify_tree(self, results: "ValidationResults") -> None:
        ...
        """
        from .compiler import _build_child_mapping
